...
public class User_Status {
	...
	public User_Status(BigInteger id,Connection conn) throws Exception{
		this.conn = conn;
		this.retrieveData(id);
	}
	public User_Status(String code,Connection conn) throws Exception{
		this.conn = conn;
		this.retrieveData(code);
	}
	private void retrieveData(BigInteger id) throws Exception{
		Statement stmt = this.conn.createStatement();
		String get_sql = String.format("SELECT * FROM User_Status \n"
	...
	private void _assignData(ResultSet rs) throws Exception{
		if(rs.getString("id") == null) throw new SQLException("User_Status not found");
	...
	public static void ensureDatabase(Connection conn) {
		try{
	    	Statement stmt = conn.createStatement();
	    	String create_table_sql = "CREATE TABLE IF NOT EXISTS User_Status (\n"
	        ...
	        for(String c:codes) {
	            ...
	            if(rs.getString("code") == null) {
	            	String create_sql = String.format("INSERT INTO User_Status (code) \n"
	                		+ "VALUES('%s');",c);
